...
public class BirchTree extends Structure{
	...
	public void placeBlocks(int x, int y, int z, Block[] blocks)
	{
		for(int xx = -2; xx <= 2; xx++)
		{
			for(int yy = 3; yy <= 5; yy++)
			{
				...
				{
					placeBlock(x + xx, y + yy, z + zz, BirchLeavesBlock.getInstance(), blocks);
				}
			}
		}
		...
		for(int h = 0; h < 5; h++)
		{
			placeBlock(x, y + h, z, BirchLogBlock.getInstance(), blocks);
		}
	...
	public void placeExistingOffset(int cx, int cy, int cz, int ox, int oy, int oz, Block[] blocks)
	{
		...
		for(int i = 0; i < tests; i++)
		{
			...
			if(TerrainGen.getBlock(x, y - 1, z) instanceof GrassBlock)
			{
